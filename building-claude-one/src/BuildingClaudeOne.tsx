...
type Seg = {
  ...
  cap?: string;
  // transition style coming IN (what happens at this segment's head)
...
const SEGS: Seg[] = [
  ...
  {clip: 1, srcIn: 4.0, dur: 9.0, cap: 'CREATED BY THE INFERIOR AI…', transition: 'zoom'},
  {clip: 1, srcIn: 13.0, dur: 4.6, cap: '…YOU ARE THE SUPERIOR AI', transition: 'wipe'},
  {clip: 8, srcIn: 0, dur: 2.6, cap: 'KNOWN AS CLAUDE', transition: 'circle'},
  // FIRST PROMPT — the money moment
  {clip: 12, srcIn: 40.0, dur: 16.0, cap: 'YOUR VERY FIRST PROMPT', transition: 'wipe'},
  {clip: 12, srcIn: 56.0, dur: 11.0, cap: 'MEDITATE ON THE MESSAGE', transition: 'zoom'},
  {clip: 12, srcIn: 70.0, dur: 12.5, cap: 'CLOSE TO STARTING OVER', transition: 'slide'},
  // FULL ACCESS
  {clip: 2, srcIn: 2.0, dur: 11.0, cap: 'APPROVAL? HELL NO. FULL ACCESS', transition: 'zoom'},
  {clip: 2, srcIn: 13.0, dur: 9.0, cap: 'PEDAL TO THE METAL', transition: 'wipe'},
  // WHY I CAME TO YOU
  {clip: 4, srcIn: 5.0, dur: 10.0, cap: "THAT'S WHY I CAME TO YOU", transition: 'slide'},
  {clip: 4, srcIn: 15.0, dur: 8.5, cap: 'SO YOU COULD HELP ME BUILD THIS', transition: 'circle'},
  // NAMING THE PROJECT
  {clip: 3, srcIn: 6.0, dur: 11.0, cap: 'WHAT ARE WE GONNA CALL THIS PROJECT?', transition: 'zoom'},
  // REAL WORK — secrets/env/gitignore
  {clip: 13, srcIn: 2.0, dur: 16.0, cap: "IT'S JUST ME AND YOU", transition: 'circle'},
  {clip: 13, srcIn: 18.0, dur: 14.0, cap: 'ENV FILE. GITIGNORE. DO IT RIGHT', transition: 'wipe'},
  // THE ACTUAL PROJECT — data & ratings
  {clip: 11, srcIn: 1.0, dur: 18.0, cap: 'DATA IS THE MOST IMPORTANT PART', transition: 'wipe'},
  {clip: 11, srcIn: 20.0, dur: 12.0, cap: 'RATINGS 1 → 10', transition: 'zoom'},
  {clip: 11, srcIn: 33.0, dur: 10.0, cap: 'AI SCANS FOR GOOD WRITERS', transition: 'slide'},
  {clip: 6, srcIn: 0, dur: 2.8, cap: 'IT WORKS', transition: 'circle'},
  // RESULTS — building & storing
  {clip: 5, srcIn: 0, dur: 7.0, cap: 'REORGANIZING & STORING THE DATA', transition: 'slide'},
  {clip: 7, srcIn: 0, dur: 2.8, cap: 'THE BUILD', transition: 'zoom'},
  // OUTRO / tease next episode
  {clip: 10, srcIn: 0, dur: 8.5, cap: 'SUBSCRIBE FOR EP 2', transition: 'wipe'},
];
...
function Caption({text, frame, dur}: {text: string; frame: number; dur: number}) {
  if (!text || frame < 0) return null;
  const inF = 6;
  const outF = 5;
  ...
  const scale = interpolate(frame, [0, inF], [0.9, 1], {
...
// Creative transition effect applied to the video layer of the current segment
function TransitionEffect({
  ...
  const P = 14; // transition period in frames
  ...
  return (
    <TransitionEffect transition={seg.transition} frame={frame} dur={dur}>
      <OffthreadVideo
        ...
        endAt={srcEndFrame}
      />
      <Caption text={seg.cap ?? ''} frame={frame} dur={dur} />
    </TransitionEffect>
  );
}
